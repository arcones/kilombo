...
def test_get_study_gse_and_srp_if_present():
...
def test_get_study_list(requests_mock):
    keyword = 'stroke AND single cell rna seq AND musculus'
    with open('fixtures/ncbi/esearch_study_list.json') as response:
        requests_mock.get(f'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=gds&retmode=json&term={keyword}', text=response.read())
    study_hierarchy = StudyHierarchy()
    NCBI(study_hierarchy).get_study_list(keyword)
    assert study_hierarchy.pending == {'200000000': {},
                                       '200126815': {},
                                       '200150644': {},
                                       '200167593': {},
                                       '200174574': {},
